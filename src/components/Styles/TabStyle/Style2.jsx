    ...
    <div class="container">
        ...
        <div class="col-12">
                ...
                {data?.tab_data?.map((item,idx)=>(     <li class="nav-item" role="presentation">
                        <Link class={`nav-link ${!idx && "active"}`} to={`#tab-2-${idx+1}`} data-bs-toggle="tab" data-bs-target={`#tab-2-${idx+1}`} role="tab" aria-selected="true">
                        {item?.tab_title}
                        </Link>
                    </li>
                    ))}
                ...
                {data?.tab_data?.map((item,idx)=>(      <div class={`tab-pane fade pt-60  ${!idx && "active show"} `} id={`tab-2-${idx+1}`} role="tabpanel">
                        <div class="row justify-content-center align-items-center justify-content-around">
                            <div class="col-lg-5">
                                <div class="feature-tab-info">
                                    ...
                                    <Link to={item?.button_url}  class="read-more-link text-decoration-none mt-4 d-block">{item?.button_text}
                                        <i class="fas fa-arrow-right ms-2"></i></Link>
                                </div>
                            </div>
                            <div class="col-lg-5">
                                <img src={`${import.meta.env.VITE_REACT_APP_IMAGE_PATH}/${item?.tab_image}`} alt={item?.tab_image_alt_tag}   class="img-fluid mt-4 mt-lg-0 mt-xl-0"/>
                            </div>
                        </div>
                    </div>
                   ))}
